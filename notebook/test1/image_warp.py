...
def get_coordinates(img_path):
    ...
    coord_path = os.path.join(
        os.path.join(*img_path.split("/")[0:-1]), "asf/{}.asf".format(img_name)
    )
    ...
    # print("x: "+  x_values + "\n y: " + y_values, connect_from, connect_to)
...
def WarpProcessing(inIm, inArr, outArr, inTriangle, triAffines, shape):
    ...
    for i in range(xmini, xmaxi):
        for j in range(ymini, ymaxi):
            ...
            if i < 0 or i >= outArr.shape[1]:
                continue
            if j < 0 or j >= outArr.shape[0]:
                continue
...
def PiecewiseAffineTransform(srcIm, srcPoints, dstIm, dstPoints):
    ...
    # print xmin, xmax, ymin, ymax
    # Determine which tesselation triangle contains each pixel in the shape norm image
    inTessTriangle = np.ones(dstIm.size, dtype=np.int64) * -1
    for i in range(int(xmin), int(xmax + 1.0)):
        for j in range(int(ymin), int(ymax + 1.0)):
    ...
    triAffines = []
    for i, simplex in enumerate(tess.simplices):
        # Extract vertices of the current triangle
        srcTriangle = srcPoints[simplex]
        dstTriangle = dstPoints[simplex]

        meanVertPos = np.hstack((srcTriangle, np.ones((3, 1)))).transpose()
        shapeVertPos = np.hstack((dstTriangle, np.ones((3, 1)))).transpose()
    ...
    targetArr = targetArr.reshape(
        targetArr.shape[0], targetArr.shape[1], len(dstIm.mode)
    )

    # Calculate pixel colours
    warp_out = WarpProcessing(
        srcIm, srcArr, targetArr, inTessTriangle, triAffines, dstPoints
    )
...
def apply_shape(shape, texture, shape_mean_x_coord, shape_mean_y_coord):
    xmax = shape_mean_x_coord.max()
    xmin = shape_mean_x_coord.min()
    ymax = shape_mean_y_coord.max()
    ymin = shape_mean_y_coord.min()
    xmini = int(xmin)
    xmaxi = int(xmax + 1.0)
    ymini = int(ymin)
    ymaxi = int(ymax + 1.0)
    ...
    texture_repeated = np.concatenate(
        (texture, np.zeros_like(texture), np.zeros_like(texture)), axis=2
    )
    texture_repeated = (texture_repeated - texture_repeated.min()) / (
        texture_repeated.max() - texture_repeated.min()
    )
...
def normalize_shape(
    individual_img_path, target_img_path, show=False, get_location=False
):
    # Load source image
...
#     normalize_shape(
#         individual_img_path="../imm3943/IMM-Frontal Face DB SMALL/11_07.jpg",
#         target_img_path="../imm3943/IMM-Frontal Face DB SMALL/08_01.jpg",
#         show=True,
#     )
